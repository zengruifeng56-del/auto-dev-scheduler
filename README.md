...
> ⚠️ **Token 开销提醒**
>
> 使用调度器会产生**额外 token 消耗**。每个 Worker 是独立的 Claude 进程，上下文无法共享：
> - 每个任务启动时都要重新读取项目配置（CLAUDE.md、openspec/）
> - 每个 Worker 需要独立理解代码库结构
> - N 个并发 Worker ≈ N 倍的项目理解开销
>
> **权衡**：用更多 token 换取更快的开发速度。如果 token 预算有限，建议减少并发数或使用单 Claude 顺序执行。
...
- 内存状态管理，任务完成自动更新 AUTO-DEV.md checkbox
- 自动管理任务依赖和执行顺序
...
1. 停止调度器
2. 在 AUTO-DEV.md 中添加 `### XX-YY: 任务名` 块
3. 重启调度器加载新任务

> 注：运行时状态由调度器内存管理，任务完成后自动更新文件中的 checkbox。
...
新架构改为**内存状态管理**：调度器在内存中维护任务状态，Worker 完成任务后通过 IPC 通知调度器更新状态并自动更新 AUTO-DEV.md 中的 checkbox，避免文件级冲突。
...
- 修复 slow tool 超时被覆盖：当长时间工具（codex/gemini）执行时，短时间工具不再覆盖超时设置

**架构改进**

- **自动更新 checkbox**：任务成功完成后自动将 `[ ]` 更新为 `[x]`，重启后可继续执行
- **文件写入队列**：多 Worker 并发完成时，通过队列串行化写入避免冲突
