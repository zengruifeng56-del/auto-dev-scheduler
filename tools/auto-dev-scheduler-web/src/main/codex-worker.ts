 ...
 * Codex Worker - Direct CLI invocation for backend/debug tasks
 *
 * Wraps the `codex exec` CLI command for:
 * - Backend logic implementation
 * - Debug analysis
 * - Code review
 *
 * Design: Uses read-only sandbox by default, passes prompt via stdin to avoid
...
// ============================================================================
// CodexWorker Implementation
// ============================================================================

export class CodexWorker extends EventEmitter implements IWorker {
  readonly id: string;
  ...
  constructor(workerId?: number) {
    super();
    this.id = `codex-${workerId ?? randomUUID().slice(0, 8)}`;
    this.capabilities = getDefaultCapabilities('codex-cli');
  }
  ...
  async *run(req: WorkerRunRequest): AsyncIterable<WorkerEvent> {
    ...
    try {
      // Build codex exec command - use stdin for prompt to avoid length limits
      const args = [
        'exec',
        '--cd', req.projectRoot,
      ...
      this.emitLog('system', `Starting Codex: codex ${args.join(' ')}`);
      ...
      this.emitLog('system', `Codex completed: ${success ? 'OK' : 'FAIL'} in ${Math.round(durationMs / 1000)}s`);
    } catch (err) {
      ...
      this.emitLog('error', `Codex error: ${error}`);
